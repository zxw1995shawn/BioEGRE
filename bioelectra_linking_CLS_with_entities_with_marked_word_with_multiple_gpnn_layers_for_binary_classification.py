...
binary_task_num_labels = {
    "gad": 2,
}

def binary_convert_examples_to_features(
    ...
    return _binary_convert_examples_to_features(
...
def _binary_convert_examples_to_features(
    ...
    if task is not None:
        processor = binary_processors[task]()
...
@dataclass
class BinaryDataTrainingArguments:
    """
...
class BinaryDataset(Dataset):
    args: BinaryDataTrainingArguments
    output_mode: str
    ...
    def __init__(
        ...
        self.args = args
        self.processor = binary_processors[args.task_name]()
        self.output_mode = binary_output_modes[args.task_name]
        ...
        with FileLock(lock_path):
            ...
            if os.path.exists(cached_features_file) and not args.overwrite_cache:
                ...
                self.features = binary_convert_examples_to_features(
                    examples,
                    tokenizer,
...
class ElectraForRelationClassification(ElectraPreTrainedModel):
    def __init__(self, config):
        ...
        self.classifier = RelationClassificationHead(config)
...
def main():
    ...
    parser = HfArgumentParser((ModelArguments, BinaryDataTrainingArguments, TrainingArguments))
    ...
    try:
        num_labels = binary_task_num_labels[data_args.task_name]
        output_mode = binary_output_modes[data_args.task_name]
    except KeyError:
    ...
    # Get datasets
    train_dataset = (
        BinaryDataset(data_args, tokenizer=tokenizer, mode="train",cache_dir=model_args.cache_dir) if training_args.do_train else None
        )
    eval_dataset = (
        BinaryDataset(data_args, tokenizer=tokenizer, mode="dev", cache_dir=model_args.cache_dir)
        if training_args.do_eval
        else None
        )
    test_dataset = (
        BinaryDataset(data_args, tokenizer=tokenizer, mode="test", cache_dir=model_args.cache_dir)
