...
class GPNNPredictionOutput(NamedTuple):
    predictions: Union[np.ndarray, Tuple[np.ndarray]]
    label_ids: Optional[Union[np.ndarray, Tuple[np.ndarray]]]
    metrics: Optional[Dict[str, float]]
    pointers: Optional[np.ndarray]
    
class GPNNEvalLoopOutput(NamedTuple):
    predictions: Union[np.ndarray, Tuple[np.ndarray]]
    label_ids: Optional[Union[np.ndarray, Tuple[np.ndarray]]]
    metrics: Optional[Dict[str, float]]
...
class Trainer_GPNN:
    ...
    def get_train_dataloader(self) -> DataLoader:
        ...
        if self.train_dataset is None:
            raise ValueError("Trainer_GPNN: training requires a train_dataset.")
    ...
    def get_eval_dataloader(self, eval_dataset: Optional[Dataset] = None) -> DataLoader:
        ...
        if eval_dataset is None and self.eval_dataset is None:
            raise ValueError("Trainer_GPNN: evaluation requires an eval_dataset.")
    ...
    def create_optimizer(self):
        ...
        if self.optimizer is None:
            ...
            optimizer_cls, optimizer_kwargs = Trainer_GPNN.get_optimizer_cls_and_kwargs(self.args)
    ...
    def predict(
        ...
        return GPNNPredictionOutput(predictions=output.predictions, label_ids=output.label_ids, metrics=output.metrics, pointers=output.pointers)
    ...
    def evaluation_loop(
        ...
        """
        Prediction/evaluation loop, shared by `Trainer_GPNN.evaluate()` and `Trainer_GPNN.predict()`.
        ...
        for step, inputs in enumerate(dataloader):
            ...
            # Prediction step
            loss, logits, labels, pointers = self.prediction_step(model, inputs, prediction_loss_only, ignore_keys=ignore_keys)
            inputs_decode = inputs["input_ids"] if args.include_inputs_for_metrics else None
            if all_pointers is not None:
                all_pointers = torch.cat((all_pointers, pointers), 0)
            else:
        ...
        all_pointers = all_pointers.cpu().numpy()

        return GPNNEvalLoopOutput(predictions=all_preds, label_ids=all_labels, metrics=metrics, num_samples=num_samples, pointers=all_pointers)
    ...
    def prediction_step(
        ...
        return (loss, logits, labels, pointers)
    ...
    def prediction_loop(
        ...
        return GPNNPredictionOutput(predictions=preds, label_ids=label_ids, metrics=metrics)
